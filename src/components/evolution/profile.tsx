...
export default function Profile({ digimon, align = null }: ProfileProps): React.ReactElement {
    ...
    return (
        <div className="profile" style={alignStyle}>
            <img className="profile-image" src={`${IMG_URL_BASE}/${getDigimonFileName(digimon.name)}.png`} alt={digimon.name} data-id={digimon.name}/>
            {/* <span>{digimon.name}</span> */}
            { digimon.grade > 2 && 
                <img className="profile-digimon-type" src={`${IMG_URL_BASE}/${digimon.digimonType}.png`} />
            }
        </div>
    );
}